...
#ifndef BOATRACERANKPREDICTION_MLP_H
#define BOATRACERANKPREDICTION_MLP_H
...
public:
  unsigned FIRST_HIDDEN_DIM;
    // Player embeddings
    LookupParameter p_player;
    LookupParameter p_stadium;
    LookupParameter p_affiliation;
    LookupParameter p_grade;
    LookupParameter p_sex;
  ...
  MLP(Model& model,
      vector<Layer> layers) {
    FIRST_HIDDEN_DIM = layers[0].output_dim;
  ...
  void add_embeddings(Model& model,
                      unsigned player_size,
                      unsigned player_embed_dim,
                      unsigned stadium_embed_dim,
                      unsigned affi_embed_dim,
                      unsigned grade_embed_dim,
                      unsigned sex_embed_dim

  ){
      p_player = model.add_lookup_parameters(player_size, {player_embed_dim});
      p_stadium = model.add_lookup_parameters(24, {stadium_embed_dim});
      p_affiliation = model.add_lookup_parameters(18, {affi_embed_dim});
      p_grade = model.add_lookup_parameters(4, {grade_embed_dim});
      p_sex = model.add_lookup_parameters(3, {sex_embed_dim});
  }
...
#endif //BOATRACERANKPREDICTION_MLP_H
